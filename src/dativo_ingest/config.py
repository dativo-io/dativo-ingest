...
class InfrastructureRuntimeConfig(BaseModel):
    ...
    def validate_type(cls, value: str) -> str:
        ...
        if value not in supported_types:
            raise ValueError(
                "infrastructure.runtime.type must be one of: "
...
class InfrastructureConfig(BaseModel):
    ...
    def validate_resource_identifiers(
        ...
        for key, raw in value.items():
            ...
            if TERRAFORM_OUTPUT_REFERENCE_PATTERN.fullmatch(cleaned_value) is None:
                raise ValueError(
                    "infrastructure.resource_identifiers['{key}'] must reference a "
                    "{{terraform_outputs.<name>}} placeholder".format(key=key)
                )
    ...
    def validate_tags(cls, value: Dict[str, Any]) -> Dict[str, str]:
        ...
        missing = REQUIRED_INFRASTRUCTURE_TAG_KEYS - set(cleaned.keys())
        if missing:
            raise ValueError(
                "infrastructure.tags is missing required keys: "
                + ", ".join(sorted(missing))
            )
        for required in REQUIRED_INFRASTRUCTURE_TAG_KEYS:
            if not cleaned[required]:
                raise ValueError(
                    f"infrastructure.tags['{required}'] must not be empty"
                )
        return cleaned
    ...
    def validate_runtime_alignment(self) -> "InfrastructureConfig":
        ...
        if missing_identifiers:
            raise ValueError(
                "infrastructure.resource_identifiers is missing required keys for "
                f"{self.provider}/{self.runtime.type}: "
                + ", ".join(sorted(missing_identifiers))
            )

        if spec.get("supports_region", True) and not self.region:
            raise ValueError(
                f"infrastructure.region is required for runtime {self.runtime.type}"
            )

        return self

    def required_resource_identifiers(self) -> List[str]:
        """Return required resource identifiers for this runtime."""
        spec = get_runtime_spec(self.provider, self.runtime.type)
...
class JobConfig(BaseModel):
    ...
    def validate_source_target(self) -> "JobConfig":
        ...
        # Ensure infrastructure metadata aligns with environment defaults
        job_environment = self.environment or "prod"
        infra_environment = self.infrastructure.tags.get("environment")
        if infra_environment and infra_environment != job_environment:
            raise ValueError(
