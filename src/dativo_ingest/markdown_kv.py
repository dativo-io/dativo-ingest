...
def parse_key_value_pairs(content: str) -> List[Dict[str, Any]]:
    ...
    for line in lines:
        ...
        if kv_match:
            ...
            current_value = [kv_match.group(2).strip()] if kv_match.group(2).strip() else []
...
def parse_to_row_per_kv(content: str, doc_id: str, extracted_at: Optional[datetime] = None) -> List[Dict[str, Any]]:
...
def parse_to_document_level(content: str, doc_id: str, extracted_at: Optional[datetime] = None) -> Dict[str, Any]:
...
def parse_markdown_kv(content: str, doc_id: str, pattern: str = "row_per_kv", extracted_at: Optional[datetime] = None) -> Any:
    ...
    if pattern == "row_per_kv":
        ...
        raise ValueError(f"Unknown pattern: {pattern}. Must be 'row_per_kv' or 'document_level'")


def transform_to_markdown_kv(data: Dict[str, Any], format: str = "compact", doc_id: Optional[str] = None) -> str:
