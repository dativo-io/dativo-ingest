...
class TestSandboxedRustReaderWrapper:
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_discover_calls_sandbox_execute(self, mock_sandbox_cls, tmp_path):
        ...
        sandbox_instance.execute.return_value = {
            "data": {"objects": [{"name": "table1"}], "metadata": {}}
        }
        ...
        assert sandbox_instance.execute.called
        # Verify result
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_discover_handles_error_response(self, mock_sandbox_cls, tmp_path):
        """Test that discover() raises SandboxError when sandbox returns error."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"error": "Plugin error"}
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_extract_calls_sandbox_execute(self, mock_sandbox_cls, tmp_path):
        """Test that extract() calls sandbox.execute() and yields batches."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"data": [[{"id": 1}], [{"id": 2}]]}
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_extract_handles_error_response(self, mock_sandbox_cls, tmp_path):
        """Test that extract() raises SandboxError when sandbox returns error."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"error": "Extraction failed"}
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_extract_handles_stateful_api_response(self, mock_sandbox_cls, tmp_path):
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_extract_with_state_manager(self, mock_sandbox_cls, tmp_path):
        """Test that extract() properly serializes state_manager if provided."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"data": [[{"id": 1}]]}
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_check_connection_calls_sandbox(self, mock_sandbox_cls, tmp_path):
        """Test that check_connection() calls sandbox.check_connection()."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.check_connection.return_value = {
            "success": True,
        ...
        assert sandbox_instance.check_connection.called
        # Verify result
...
class TestSandboxedRustWriterWrapper:
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_write_batch_calls_sandbox_execute(self, mock_sandbox_cls, tmp_path):
        ...
        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {
            "data": [{"path": "s3://bucket/file.parquet", "size_bytes": 1024}]
        }
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_write_batch_handles_error_response(self, mock_sandbox_cls, tmp_path):
        """Test that write_batch() raises SandboxError when sandbox returns error."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"error": "Write failed"}
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_commit_files_calls_sandbox_execute(self, mock_sandbox_cls, tmp_path):
        ...
        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {
            "data": {"status": "success", "files_committed": 2}
        }
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_commit_files_handles_error_response(self, mock_sandbox_cls, tmp_path):
        """Test that commit_files() raises SandboxError when sandbox returns error."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"error": "Commit failed"}
        mock_sandbox_cls.return_value = sandbox_instance
    ...
    @patch("dativo_ingest.rust_sandboxed_wrapper.RustPluginSandbox")
    def test_write_batch_creates_writer_if_needed(self, mock_sandbox_cls, tmp_path):
        """Test that write_batch() passes config to allow writer creation."""
        plugin_file = tmp_path / "test_plugin.so"
        plugin_file.write_bytes(b"fake plugin binary")

        sandbox_instance = Mock()
        sandbox_instance.execute.return_value = {"data": [{"path": "file.parquet"}]}
        mock_sandbox_cls.return_value = sandbox_instance
        ...
        # Verify execute was called with config payload
        assert sandbox_instance.execute.called
        _, call_kwargs = sandbox_instance.execute.call_args
