...
class TestRunSummaryArtifact:
    ...
    def job_config(self):
        ...
        mock_config.get_source.return_value = SourceConfig(
            type="csv",
            object="test_object",
            incremental={"state_path": "state.json", "cursor_field": "updated_at"},
        )
        mock_config.get_target.return_value = TargetConfig(
            type="parquet", connection={"bucket": "test_bucket"}
        )
    ...
    def test_run_summary_generated_on_success(
        ...
        mock_writer_instance.write_batch.return_value = [
            {"path": "file1.parquet", "size_bytes": 100}
        ]
        ...
        with patch.dict(os.environ, {"STATE_DIR": str(tmp_path)}):
            ...
            assert summary["run"]["tenant_id"] == "test_tenant"
            assert summary["run"]["type"] == "incremental"
            assert summary["ingestion"]["status"] == "success"

            assert summary["volume"]["records_extracted"] == 1
            assert summary["volume"]["records_written"] == 1
            assert summary["volume"]["bytes_written"] == 100

            assert summary["asset"]["id"] == "test-asset-id"
            assert summary["schema"]["version"] == "1.0.0"

            assert summary["time"]["event_time_field"] == "updated_at"
    ...
    def test_run_summary_generated_on_failure(
        ...
        with patch.dict(os.environ, {"STATE_DIR": str(tmp_path)}):
            ...
            assert summary["ingestion"]["status"] == "failure"
            assert summary["ingestion"]["error"]["has_errors"] is True
            assert (
                "Extractor init failed"
                in summary["ingestion"]["error"]["error_message"]
            )
