...
class TestExpandEnvVariable:
    ...
    def test_expand_simple_var(self, monkeypatch):
        ...
        result = expand_env_variable("${BUCKET}")
    ...
    def test_expand_var_with_default_not_set(self):
        ...
        result = expand_env_variable("${ENV:-prod}")
        assert result == "prod"
    ...
    def test_expand_embedded_var_with_default(self):
        ...
        result = expand_env_variable("my-bucket-${ENV:-prod}")
    ...
    def test_expand_mixed_var_types(self, monkeypatch):
        ...
        result = expand_env_variable("${BUCKET}-${ENV:-prod}")
    ...
    def test_expand_none_value(self):
        ...
        result = expand_env_variable(None)
    ...
    def test_expand_non_string_value(self):
        ...
        result = expand_env_variable(123)
        assert result == 123

        result = expand_env_variable({"key": "value"})
    ...
    def test_cli_expand_env_variable_same_behavior(self, monkeypatch):
        """Test that utils version works correctly (previously tested CLI vs catalog)."""
        monkeypatch.setenv("ENV", "dev")
        from dativo_ingest.utils import expand_env_variable
        ...
        for test_case in test_cases:
            result = expand_env_variable(test_case)
            # Should expand ENV=dev, so result should contain "dev"
            assert (
                "dev" in result or result == "dev"
            ), f"Failed to expand '{test_case}': result={result}"
