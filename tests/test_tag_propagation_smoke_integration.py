...
def test_smoke_basic_tag_propagation_flow(
    sample_asset_with_tags, target_config_iceberg
):
...
def test_smoke_tag_propagation_with_job_overrides(
    sample_asset_with_tags, target_config_iceberg
):
...
def test_smoke_table_property_updates_idempotent(
    sample_asset_with_tags, target_config_iceberg
):
...
def test_smoke_property_merging_preserves_existing(
    sample_asset_with_tags, target_config_iceberg
):
...
def test_smoke_tag_namespace_verification(
    sample_asset_with_tags, target_config_iceberg
):
    ...
    classification_keys = [
        k for k in properties.keys() if k.startswith("classification.")
    ]
