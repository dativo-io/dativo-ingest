...
- Synthetic data job: `tests/fixtures/jobs/mimesis_perf_test.yaml`
...
Performance tests use a **1GB CSV file** generated by the synthetic `mimesis` connector job:

```bash
# Generate data with the connector (writes to /data/synthetic_perf.csv by default)
python3 -m dativo_ingest.cli run \
  --config tests/fixtures/jobs/mimesis_perf_test.yaml \
  --secrets-dir tests/fixtures/secrets \
  --mode self_hosted
```

The generated CSV file is stored at:
- Default: `/data/synthetic_perf.csv`
...
export PERF_TEST_CSV_FILE=/data/synthetic_perf.csv
...
```bash
# Regenerate using the synthetic connector job
python3 -m dativo_ingest.cli run \
  --config tests/fixtures/jobs/mimesis_perf_test.yaml \
  --secrets-dir tests/fixtures/secrets \
  --mode self_hosted
