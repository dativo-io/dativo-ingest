...
def generate_csv_file(output_path: Path, target_size_gb: float = 1.0):
    ...
    columns = [
        ...
        'age', 'city', 'country', 'status', 'created_at'
    ]
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        while bytes_written < target_size_bytes:
            ...
            row = [
                row_id, name, email, department, salary,
                age, city, country, status, created_at
            ]
            ...
            if row_count % 100000 == 0:
                ...
                print(f"  Generated {row_count:,} rows ({size_mb:.2f} MB, {progress:.1f}%)...")
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help='Target size in GB (default: 1.0)'
    )
    parser.add_argument(
        '--output',
        type=str,
    ...
    if args.output:
        ...
        project_root = Path(__file__).parent.parent.parent
        output_path = project_root / "tests" / "fixtures" / "seeds" / "perf_test_data_1gb.csv"
